...
RESOURCE_META=$(curl -sf "${PROXY_URL}/.well-known/oauth-protected-resource") \
...
HTTP_CODE=$(curl -sf -o /dev/null -w "%{http_code}" "${PROXY_URL}/api/v1/adapters" 2>/dev/null || true)
WWW_AUTH=$(curl -sf -D - -o /dev/null "${PROXY_URL}/api/v1/adapters" 2>/dev/null | grep -i "www-authenticate" || true)
